"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PartyTaxScheme = void 0;
const xml_serializer_ts_1 = require("xml-serializer-ts");
const constants_1 = require("@fe/common/constants");
const TaxScheme_1 = require("./TaxScheme");
const CompanyID_1 = require("./CompanyID");
const RegistrationAddress_1 = require("./RegistrationAddress");
const { CAC, CBC } = constants_1.prefix;
let PartyTaxScheme = class PartyTaxScheme {
    constructor(pts) {
        this.registrationName = pts.registrationName;
        this.companyID = pts.companyID;
        this.taxScheme = pts.taxScheme;
        this.registrationAddress = pts.registrationAddress;
    }
};
__decorate([
    xml_serializer_ts_1.XMLChild({ namespace: CBC, name: 'RegistrationName' }),
    __metadata("design:type", String)
], PartyTaxScheme.prototype, "registrationName", void 0);
__decorate([
    xml_serializer_ts_1.XMLChild({ namespace: CBC, name: 'CompanyID' }),
    __metadata("design:type", CompanyID_1.CompanyID)
], PartyTaxScheme.prototype, "companyID", void 0);
__decorate([
    xml_serializer_ts_1.XMLChild({ namespace: CAC, name: 'TaxScheme' }),
    __metadata("design:type", TaxScheme_1.TaxScheme)
], PartyTaxScheme.prototype, "taxScheme", void 0);
__decorate([
    xml_serializer_ts_1.XMLChild({ namespace: CAC, name: 'RegistrationAddress' }),
    __metadata("design:type", RegistrationAddress_1.RegistrationAddress)
], PartyTaxScheme.prototype, "registrationAddress", void 0);
PartyTaxScheme = __decorate([
    xml_serializer_ts_1.XMLElement({ root: CAC }),
    __metadata("design:paramtypes", [PartyTaxScheme])
], PartyTaxScheme);
exports.PartyTaxScheme = PartyTaxScheme;
//# sourceMappingURL=PartyTaxScheme.js.map